...
"""
    ...
    @torch.no_grad()
    def calculate_electome_contributions(self, X):
        """
        Calculate the contribution of each electome to the final output for each sample.

        Parameters:
        X : Input data (number of samples, number of features)

        Returns:
        contributions: Raw contributions from each electome (number of samples, number of electomes)
        normalized_contributions: Contributions normalized to sum to 1 (number of samples, number of electomes)
        """
        # Ensure input is on the correct device
        X = torch.Tensor(X).to(self.device)

        # Get the latent representation (s) and the predicted probabilities
        y_pred_proba, s = self.predict_proba(X, include_scores=True)

        # Initialize a list to collect contributions
        contributions = []

        # Loop over each electome to calculate its contribution
        for sup_net in range(self.n_sup_networks):
            # Ensure electome_activation is a tensor
            electome_activation = torch.Tensor(s[:, sup_net]).to(self.device)

            # Get the weight (phi) for the current electome and ensure it's a tensor
            weight = torch.tensor(self.get_phi(sup_net).item()).to(self.device)

            # Handle multiple intercepts: Use the average intercept and ensure it's a tensor
            bias = torch.tensor(self.beta_list[sup_net].mean().item()).to(self.device)

            # Compute the raw contribution to the predicted probability
            contribution = torch.sigmoid(electome_activation * weight + bias)

            # Store the contribution
            contributions.append(contribution.view(-1, 1))

        # Concatenate all contributions along the second dimension
        contributions = torch.cat(contributions, dim=1)

        # Normalize the contributions so they sum to 1 for each sample
        total_contributions = torch.sum(contributions, dim=1, keepdim=True)
        normalized_contributions = contributions / total_contributions

        return contributions.cpu().numpy(), normalized_contributions.cpu().numpy()
